export function requestFunction(method,conv,senderAddingToMessageBox,receiverAddingToMessageBox,loader,u1,u2){
    ...
    if(method=='post'){
        ...
        let senderName=conv.senderId.value;
        let recipientName=conv.recipientId.value;
        conv.senderId=conv.senderId.id;
        conv.recipientId=conv.recipientId.id;
        xhr.send(JSON.stringify(conv)); 
        loader.innerHTML=`<i class='fa fa-spinner fa-2x fa-spin'></i>`
        xhr.onload=function(){
               if(xhr.status==201){
                   loader.innerHTML='';
                   conv.senderId=senderName;
                   conv.recipientId=recipientName;
